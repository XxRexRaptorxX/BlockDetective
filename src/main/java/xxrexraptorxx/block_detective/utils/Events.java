            ...
            for (Block block : ForgeRegistries.BLOCKS) {
                if (ForgeRegistries.ITEMS.getKey(item) == ForgeRegistries.BLOCKS.getKey(block)) {
                    ...
                    if (!Screen.hasShiftDown()) {
                        list.add(new TranslatableComponent("message.block_detective.hold_shift.desc").withStyle(ChatFormatting.GRAY));

                    } else {
                        if (Config.SHOW_REGISTRY_NAME.get()) list.add(new TextComponent(ForgeRegistries.ITEMS.getKey(item).toString()).withStyle(ChatFormatting.GOLD));
                        if (Config.SHOW_DESTROY_TIME.get()) list.add(new TextComponent("Destroy Time: " + String.valueOf(block.defaultDestroyTime())).withStyle(ChatFormatting.YELLOW));
                        if (Config.SHOW_JUMP_FACTOR.get()) list.add(new TextComponent("Jump Factor: " + String.valueOf(block.getJumpFactor())).withStyle(ChatFormatting.YELLOW));
                        if (Config.SHOW_SPEED_FACTOR.get()) list.add(new TextComponent("Speed Factor: " + String.valueOf(block.getSpeedFactor())).withStyle(ChatFormatting.YELLOW));
                        if (Config.SHOW_LIGHT_LEVEL.get()) list.add(new TextComponent("Light Level: " + String.valueOf(block.defaultBlockState().getLightEmission())).withStyle(ChatFormatting.YELLOW));
                        if (Config.SHOW_FRICTION.get()) list.add(new TextComponent("Friction: " + String.valueOf(block.getFriction())).withStyle(ChatFormatting.YELLOW));
                        if (Config.SHOW_IF_FLAMMABLE.get()) list.add(new TextComponent("Flammable: " + FormattingHelper.ConvertBooleanToString(block.defaultBlockState().getMaterial().isFlammable())).withStyle(ChatFormatting.YELLOW));
                        if (Config.SHOW_IF_SOLID.get()) list.add(new TextComponent("Solid: " + FormattingHelper.ConvertBooleanToString(block.defaultBlockState().getMaterial().isSolid())).withStyle(ChatFormatting.YELLOW));
                    }
                }
            }
        }
        ...
        if (Config.UPDATE_CHECKER.get()) {
            if (!hasShownUp && Minecraft.getInstance().screen == null) {
                if (VersionChecker.getResult(ModList.get().getModContainerById(References.MODID).get().getModInfo()).status() == VersionChecker.Status.OUTDATED ||
                    ...
                    MutableComponent url = new TextComponent(ChatFormatting.GREEN + "Click here to update!");
